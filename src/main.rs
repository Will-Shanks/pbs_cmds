...
use pbs::{Attribs, ResvModFlag, ResvSubFlag, Server, StatResp, Status};
...
pub struct Submit {
    ...
    #[arg(
        ...
        long,
        action,
        conflicts_with = "no_attribs",
        conflicts_with = "json",
        help = "print resources over multiple lines"
    )]
    long: bool,
...
pub struct Filter {
    ...
    #[arg(
        short,
        long,
        help = "filter attributes, ex: state=free, can use =, !=, <, <=, >, and >="
    )]
    attribs: Vec<String>,
    #[arg(
        short,
        long,
        conflicts_with = "no_attribs",
        help = "attributes to display, ex: state"
    )]
    ...
    #[arg(
        ...
        action,
        conflicts_with = "out",
        help = "don't print any attributes, only names"
    )]
    no_attribs: bool,
    #[arg(
        short,
...
impl Filter {
    ...
    fn check(&self, status: &Status) -> bool {
        ...
        if !status.attribs().check_filter(&self.attribs()) {
            return false;
        }
        true
    }
    fn printfmt(&self) -> Printfmt {
        if self.json {
            return Printfmt::Json;
        }
        if self.long {
            return Printfmt::Long;
        }
        Printfmt::Long
        //Printfmt::Short
    }
            ...
            let out: Vec<_> = data
                .map(|item| {
                    ...
                    attribs
                        .as_object_mut()
                        .unwrap()
                        .insert("name".to_string(), serde_json::Value::String(item.name()));
                    attribs
                })
                .collect();
        ...
        Noun::Reservation(verb) => {
            match verb {
                ...
                ResvVerb::Sub(attribs) => {
                    ...
                    if attribs.maintenance {
                        flags.push(ResvSubFlag::Maintenance)
                    };
